  ...
  return (
    ...
    <Navbar/>
      <div className='flex bg-[#2D3B58] pt-9'>
        {!loading ?<div className='bg-[#2D3B58] h-[calc(100vh-2.5rem)] w-full flex flex-col '>
        <div className='flex flex-col w-screen bg-[#2D3B58] md:px-16 pt-6 text-white lg:w-[95vw]'  >
          <div className='flex lg:mx-36'>
                <div className='p-1 pl-2  md:px-8 flex flex-col items-center'>
                      <img className='rounded-full w-[80px] lg:w-[120px] '  src={user?.profile} />
        ...
        {user?.postCount !== 0 ?<div className='md:px-12 lg:px-12 bg-[#2D3B58] '>
        <div className='m-0 w-[50vw] md:w-[80vw] p-9 bg-[#2D3B58] overflow-y-scroll md:h-[74%]  justify-center md:grid md:absolute hidden auto-rows-2fr grid-cols-8' >
                  {post.map((p) =>(
                      <Pin url={p.content} id={p._id} key={p._id} size={sizeArray[Math.floor(Math.random() * 3)]}  />
                    ))}
        </div>
         {!sloading? <div className='md:hidden px-4 py-2'>
